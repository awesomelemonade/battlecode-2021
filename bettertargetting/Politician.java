package bettertargetting;

import battlecode.common.*;
import bettertargetting.util.Cache;
import bettertargetting.util.Constants;
import bettertargetting.util.LatticeUtil;
import bettertargetting.util.MapInfo;
import bettertargetting.util.Pathfinder;
import bettertargetting.util.UnitCommunication;
import bettertargetting.util.Util;
...
public boolean goToECs() {
        ...
        Comparator<MapLocation> compareECs = Comparator.comparingInt((MapLocation loc) -> (MapInfo.getKnownEnlightenmentCenterList(Constants.ALLY_TEAM)
                .getClosestLocationDistance(loc, 1024) + MapInfo.getKnownEnlightenmentCenterList(Constants.ENEMY_TEAM)
        ...
        if (bestEnemyEC == null) {
            ...
            best = bestEnemyEC;
        } else if (compareECs.compare(bestEnemyEC, bestNeutralEC) <= 0) {
            best = bestEnemyEC;
        } else {
            best = bestNeutralEC;
        }
        ...
        if (!shouldAttack(best)) return false;
        Pathfinder.execute(best);
        return true;
    }
