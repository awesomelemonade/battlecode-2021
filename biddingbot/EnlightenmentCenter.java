...
import biddingbot.util.Cache;
import biddingbot.util.CentralUnitTracker;
import biddingbot.util.Constants;
import biddingbot.util.LambdaUtil;
import biddingbot.util.MapInfo;
import biddingbot.util.SlandererBuild;
import biddingbot.util.Util;
...
public strictfp class EnlightenmentCenter implements RunnableBot {
    ...
    private static int idealBid = 1;
    private static int lastVotes = -1;
    private static boolean wonLastBid = true;
    private static boolean[] recentBidTracker = new boolean[10];
    private static int bidTrackerIdx = 0;
    ...
    public void preTurn() {
        ...
        if(rc.getTeamVotes() > lastVotes) {
            lastVotes = rc.getTeamVotes();
            wonLastBid = true;
        } else {
            wonLastBid = false;
        }
        recentBidTracker[bidTrackerIdx] = wonLastBid;
        bidTrackerIdx++; bidTrackerIdx %= 10;
    }
    ...
    public static void bid(int influence) throws GameActionException {
        ...
        if(influence <= 50 && rc.getRoundNum() <= 50) {
            return;
        }

        int minBid = Math.max(1, (int) (0.1 * rc.getInfluence() / (1500 - rc.getRoundNum())));

        // whichever is more: 1% of total influence or 30% of this turn's ration
        int maxBid = Math.max((int) (0.3 * rc.getInfluence() / (1500 - rc.getRoundNum())), (int)(0.02 * rc.getInfluence()));
        if(maxBid < minBid) maxBid = minBid;

        int numWinsInLast10Rounds = 0;
        for(int i = 10; --i >= 0;) {
            if(recentBidTracker[i]) numWinsInLast10Rounds++;
        }
        if(numWinsInLast10Rounds >= 6) {
            if(rc.getRoundNum()%3 == 0) {
                idealBid = Math.min(idealBid-1, (int)(0.9*idealBid));
            }
        } else {
            idealBid = Math.max(idealBid + 5, 2 * idealBid);
        }
        if(idealBid < minBid) idealBid = minBid;
        if(idealBid > 2*maxBid+10) idealBid = minBid; // way out of our price range, might as well bid minimum amount
        if(idealBid > maxBid) idealBid = maxBid;
        rc.bid(idealBid);
    }
