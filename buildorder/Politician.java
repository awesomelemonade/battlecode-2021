...
import buildorder.util.LambdaUtil;
import buildorder.util.LatticeUtil;
import buildorder.util.MapInfo;
import buildorder.util.Pathfinder;
import buildorder.util.UnitCommunication;
...
public strictfp class Politician implements RunnableBot {
    ...
    public boolean shouldEmpower(int ecKills, int mKills, int pKills, int convictionGotten) throws GameActionException {
        if (ecKills >= 1) {
            return true;
        }
        ...
        if (rc.getConviction() <= 30) {
            if (numKills == 0)
                return false;
            if (nearestS != null && nearestS.distanceSquaredTo(Cache.MY_LOCATION) <= 100 && mKills >= 1)
                return true;
    ...
    public boolean tryEmpower() throws GameActionException {
        ...
        if (shouldEmpower(ecKills, mKills, pKills, convictionGotten)) {
