...
import combobot.util.Cache;
import combobot.util.Constants;
import combobot.util.LambdaUtil;
import combobot.util.MapInfo;
...
public strictfp class Politician implements RunnableBot {
    ...
    public static long getMultiEmpowerScore(MapLocation location, int radiusSquared) {
        ...
        // totalDamageValue / currentConviction_10 <= 0.3
        if (totalDamageValue * 10 <= 3 * currentConviction_10) {
            return Long.MIN_VALUE;
        }
    ...
    public static boolean tryEmpowerLatticebot() throws GameActionException {
        if (currentDamage <= 0) {
            return false;
        }
        if (Cache.ENEMY_ROBOTS.length + Cache.NEUTRAL_ROBOTS.length == 0) {
    ...
    private static int getScore(int radiusSquared) throws GameActionException {
        ...
        for (int i = enemyRobots.length; --i >= 0; ) {
            ...
            if (enemy.getType() == RobotType.POLITICIAN) {
                pConviction += getDamageValue(damage, enemy);
