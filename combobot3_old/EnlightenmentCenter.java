...
import combobot3_old.util.CentralUnitTracker;
import combobot3_old.util.Constants;
import combobot3_old.util.LambdaUtil;
import combobot3_old.util.MapInfo;
import combobot3_old.util.SlandererBuild;
...
public strictfp class EnlightenmentCenter implements RunnableBot {
    ...
    public static void bid(int influence) throws GameActionException {
        ...
        int amount;
        if (r < 0.2) {
            amount = Math.max(biddingbot.util.Util.randBetween(1, 3), (int) (0.1 * rc.getInfluence() / (1500 - rc.getRoundNum())));
        } else if (r < 0.4) {
