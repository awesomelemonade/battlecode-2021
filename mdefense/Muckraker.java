...
public strictfp class Muckraker implements RunnableBot {
    ...
    public static boolean protectEC() throws GameActionException {
        // find square that is cardinal direction from ally ec
        int ecConv = -1;
        MapLocation dest = null;
        int bestDist = 9999;
        RobotInfo[] allies = rc.senseNearbyRobots(2, Constants.ALLY_TEAM);
        for(RobotInfo robot : allies) {
            if(robot.getType() == RobotType.ENLIGHTENMENT_CENTER) {
                MapLocation robotloc = robot.location;
                for(Direction dir : Constants.CARDINAL_DIRECTIONS) {
                    MapLocation loc = robotloc.add(dir);
                    if(rc.canSenseLocation(loc) && (!rc.isLocationOccupied(loc) || loc.equals(Cache.MY_LOCATION))) {
                        int dist = loc.distanceSquaredTo(Cache.MY_LOCATION);
                        if(dist < bestDist) {
                            bestDist = dist;
                            dest = loc;
                            ecConv = robot.getConviction();
                        }
                    }
                }
            }
        ...
        // check if there is dangerous big p nearby
        boolean hasEnemy = false;
        for(RobotInfo robot : Cache.ENEMY_ROBOTS) {
            if (robot.getType() == RobotType.POLITICIAN) {
                if (2*(robot.getConviction()-10) >= ecConv || robot.getConviction() >= 50) {
                    hasEnemy = true;
                    break;
                }
            }
        ...
        if(dest.equals(Cache.MY_LOCATION)) return true;
        return Util.tryMove(dest);
    }
