...
public class Pathfinder {
    ...
    public static boolean executeSpacedApart(MapLocation target) {
        ...
        for (int i = Cache.ALLY_ROBOTS.length; --i >= 0; ) {
            ...
            System.out.println(allyLocation.x + " " + allyLocation.y);
            // force vectors
            if (ally.getType() == RobotType.MUCKRAKER) {
                ...
                x -= 5 * (allyLocation.x - Cache.MY_LOCATION.x) / distanceCubed;
                y -= 5 * (allyLocation.y - Cache.MY_LOCATION.y) / distanceCubed;
            }
        }
        MapLocation targetLocation = new MapLocation((int) Math.round(x), (int) Math.round(y));
        Util.setIndicatorLine(Cache.MY_LOCATION, targetLocation, 255, 0, 0);
        if(targetLocation.equals(Cache.MY_LOCATION)) return execute(target);
        return execute(targetLocation);
    }
}
