...
import trollbigps.util.Cache;
import trollbigps.util.Constants;
import trollbigps.util.LambdaUtil;
import trollbigps.util.MapInfo;
import trollbigps.util.Pathfinder;
import trollbigps.util.UnitCommunication;
import trollbigps.util.Util;
import trollbigps.util.EnlightenmentCenterList.EnlightenmentCenterListNode;
...
public strictfp class Politician implements RunnableBot {
    ...
    public void init() throws GameActionException {
        ...
        if (Math.random() < 0.05 && rc.getConviction() >= 100) {
            troll = true;
        } else {
            troll = false;
        }
    ...
    private static int getScore(int radiusSquared) throws GameActionException {
        ...
        int score = numKills * 100000 + mConviction + ecConviction;
    ...
    private static Comparator<EnlightenmentCenterListNode> farthestEC = Comparator.comparingInt(
            (EnlightenmentCenterListNode ec) -> {
                return -ec.location.distanceSquaredTo(allyECCentroid);
            }
    );



    private static void doTroll() throws GameActionException {
        // if already adjacent, don't move
        for (Direction d : Constants.ORDINAL_DIRECTIONS) {
            MapLocation adj = Cache.MY_LOCATION.add(d);
            if(!rc.canSenseLocation(adj)) continue;
            RobotInfo robot = rc.senseRobotAtLocation(adj);
            if(robot != null && robot.getTeam() == Constants.ENEMY_TEAM && robot.getType() == RobotType.ENLIGHTENMENT_CENTER) {
                return;
            }
        }
        //EnlightenmentCenterListNode farthestEnemyEC = MapInfo.getKnownEnlightenmentCenterList(Constants.ENEMY_TEAM).min(farthestEC).orElse(null);
        ...
        if(ecLoc != null) {
            MapLocation loc = ecLoc;
            int dist = loc.distanceSquaredTo(Cache.MY_LOCATION);
            if(dist <= 13) { // can see all adjacent squares
                if(dist <= 2) return; // already adjacent
                for (Direction d : Constants.ORDINAL_DIRECTIONS) {
