...
import waves.util.Cache;
import waves.util.Constants;
import waves.util.LambdaUtil;
import waves.util.LatticeUtil;
import waves.util.MapInfo;
import waves.util.Pathfinder;
import waves.util.UnitCommunication;
import waves.util.Util;
...
public strictfp class Politician implements RunnableBot {
    ...
    public void turn() throws GameActionException {
        ...
        if (rc.getConviction() >= 50 && tryHealEC()) {
        ...
        if (selfempowerer && rc.getConviction() >= 50 && trySelfBuff()) {
        ...
        if (((defender && rc.getConviction() < 50) || !shouldWave()) && tryDefend()) {
    ...
    public boolean goToNearestEC() {
        MapLocation closestNeutral = MapInfo.getKnownEnlightenmentCenterList(Team.NEUTRAL).getClosestLocation(Cache.MY_LOCATION).orElse(null);
        if(closestNeutral != null) {
            Pathfinder.execute(closestNeutral);
            return true;
        }
        if(!shouldWave()) return false;
        MapLocation closestEnemy = MapInfo.getKnownEnlightenmentCenterList(Constants.ENEMY_TEAM).getClosestLocation(Cache.MY_LOCATION).orElse(null);
        if(closestEnemy != null) {
            Pathfinder.execute(closestEnemy);
            return true;
        }
        return false;
    }
